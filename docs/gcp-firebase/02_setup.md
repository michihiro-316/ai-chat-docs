...
!!! note "システムの流れ"
    ...
    | ステップ | 処理 |
    |---------|------|
    | (1) | あなた（ブラウザ）→ アクセス → Cloud Run functions |
    | (2) | Cloud Run functions →「ログインしてる？」→ Firebase Auth（認証） |
    | (3) | Firebase Auth ← Googleアカウントでログイン → ログイン済みならOK |
    | (4) | Cloud Run functions → APIキーを取得 → Secret Manager（鍵の金庫） |
    | (5) | Cloud Run functions → AI に質問 → OpenAI API（ChatGPT） |
    | (6) | OpenAI API → 回答 → あなた（ブラウザ）に表示 |
...
!!! info "FirebaseとGCPの関係"

    Firebaseでプロジェクトを作成すると、**GCPプロジェクトも自動的に作成**されます。
    別々に作成する必要はありません。

    1. Firebaseでプロジェクト作成
    2. 自動的にGCPプロジェクトも作成される
    3. 両方のコンソールで同じプロジェクトを操作できる
...
!!! info "「公開アクセスを許可する」を選択する理由"

    **2段階の認証になっています：**

    | 段階 | 認証レベル | 説明 |
    |-----|----------|------|
    | **1段階目** | Cloud Run の認証（GCPレベル） | 「公開アクセスを許可する」= 誰でもURLにアクセスできる = ログイン画面を表示するため、ここは許可が必要。※「認証が必要」を選ぶと、GCPアカウントがないとログイン画面すら見れなくなる |
    | **2段階目** | アプリ側の認証（Firebase Auth） | ログイン画面で「Googleでログイン」ボタンを押す → ログインした人だけがチャット機能を使える。※ここで本当のアクセス制限をかけている |
...
!!! tip "インスタンスとは？"
    ...
    **分かりやすい例え：インスタンス = ファストフード店のレジ係**

    | 状況 | 対応 |
    |------|------|
    | お客さん数人 | レジ係1人で対応できる |
    | お客さん100人が殺到 | レジ係を10人に増やして対応（自動！） |
    | お客さん0人（深夜など） | レジ係を帰宅させてコスト削減 |
    ...
    **計算例：**
    インスタンス最大数: 10 × 1インスタンスあたり: 80リクエスト = **最大800リクエストを同時に処理可能**

!!! info "「1に設定するとコールド スタートが減少します」とは？"
    ...
    **最小=0 の場合（コールドスタート）:**
    ユーザーがアクセス → インスタンスが停止中...（誰もいない店）→ 起動開始（店を開けて準備中）→ 2〜5秒後に応答 ← この待ち時間が「コールドスタート」

    **最小=1 の場合（ウォームスタート）:**
    ユーザーがアクセス → インスタンスは起動済み！（店はすでに営業中）→ すぐに応答
...
!!! info "「サービスのスケーリング」と「リビジョン スケーリング」の違い"

    | 設定 | 場所 | 説明 |
    |------|------|------|
    | **サービスのスケーリング** | 画面上部 | サービス全体のインスタンス数を設定。通常はこちらで設定すればOK |
    | **リビジョン スケーリング** | コンテナ設定内 | 特定のリビジョン（バージョン）のインスタンス数。複数バージョンを同時運用する場合に使用。通常は上部の設定と同じ値でOK |
...
!!! tip "環境変数とは？"
    ...
    |------|------|
    | コード（変えない） | 「許可リストをチェック」 |
    | 環境変数（変える） | 許可リスト = `user1@example.com` |

    ※ 許可するユーザーを追加したい時、コードは変えずに環境変数だけ変更すればOK
...
!!! note "なぜ環境変数でアクセス制限ができるのか？（仕組みの解説）"

    | ステップ | 処理 |
    |---------|------|
    | 1 | ユーザーがGoogleでログインする |
    | 2 | Googleから「この人は tanaka@yourcompany.co.jp です」という情報が返ってくる |
    | 3 | フロントエンド（front.html）がバックエンド（backend.js）に確認リクエストを送る「このメールアドレスの人は使っていいですか？」 |
    | 4 | バックエンド（backend.js）が環境変数をチェック: ALLOWED_EMAILS に含まれているか？→ NO、ALLOWED_DOMAINS で終わっているか？→ YES（@yourcompany.co.jp で終わる） |
    | 5 | 結果を返す: 許可 → チャット画面を表示、拒否 →「アクセスが許可されていません」画面を表示 |
>
...
!!! tip "エントリポイントとは？"
    ...
    プログラムの「入口」です。リクエストが来た時に**最初に実行される関数の名前**を指定します。

    1. ユーザーがアクセス
    2. Cloud Run:「エントリポイントは 'app' だな」
    3. コード内の `functions.http('app', ...)` を実行
    4. 処理開始
>
> **コードとの対応（backend.js）**
>
...
> | 設定箇所 | 設定値 | 説明 |
> |---------|--------|------|
> | GCPコンソール → エントリポイント | `app` | GCPに教える「入口の名前」 |
> | backend.js → `functions.http('app', ...)` | `app` | コード内で定義した「入口の名前」 |
>
> **この2つが一致していないとエラーになります。**
>
> ```
> エラー例: Function 'helloWorld' is not defined
...
> **よくある質問: Firebase Config をコードに書いて大丈夫？**
>
> **結論: 大丈夫です。** Firebase Config は「識別情報」であり、「認証キー」ではありません。
>
!!! info "「キー」と呼ばれるものは2種類ある"

    | 種類 | 例 | 役割 | 公開 |
    |------|-----|------|:----:|
    | 識別キー | Firebase apiKey | 「私は○○プロジェクトです」と伝える名札 | ○ OK |
    | 認証キー | OpenAI APIキー | 「この鍵を持つ人はお金を使える」という財布の鍵 | × 絶対ダメ |

!!! success "なぜ Firebase apiKey は漏れても安全？"

    apiKey だけでは何もできません。**4層の防御**があるからです。

    | 層 | チェック内容 | 結果 |
    |---|------------|------|
    | 第1層: 承認済みドメイン | evil.com からログイン？ | 拒否！ |
    | 第2層: ログイン必須 | ログインしてない？ | データにアクセス拒否！ |
    | 第3層: 許可ユーザー制限 | ALLOWED_EMAILS にいない？ | 機能使用を拒否！ |
    | 第4層: Security Rules | 他人のデータを読もうとした？ | 拒否！ |

!!! danger "一方、OpenAI APIキー は防御が0層"

    OpenAI APIキーを知っている悪い人 → 防御: なし → APIキーが正しい？ → OK！ → **お金が使われる（課金される）**

!!! note "まとめ"

    | 置く場所 | 何を置く |
    |---------|---------|
    | script.js（ブラウザで見える） | Firebase Config ← OK |
    | Secret Manager（サーバーのみ） | OpenAI APIキー ← 必須 |
...
Cloud RunのURLでGoogleログインを使用するには、**承認済みドメインへの追加が必須**です。

#### なぜ必要か？

Firebase Authenticationは、セキュリティ上の理由から**OAuth認証を許可するドメインを制限**しています。

!!! warning "承認済みドメインがない場合"

    1. 悪意のあるサイト（evil.com）
    2. あなたのFirebase認証を埋め込もうとする
    3. Firebase:「evil.comは許可リストにないので拒否！」
    4. ユーザーの認証情報が盗まれることを防止

つまり、**あなたのCloud Run URLも許可リストに追加しないと、正規のサイトでもログインできません**。
...
2. **「承認済みドメイン」** タブをクリック
3. **「ドメインを追加」** をクリック
4. Cloud RunのURL（ホスト名部分のみ）を入力:
   ```
   ai-chat-app-xxxxx-an.a.run.app
   ```
   ※ `https://` は含めない
5. 「追加」をクリック
...
> |---------|------|
> | `localhost` | ローカル開発用 |
> | `xxx.firebaseapp.com` | Firebaseのデフォルトドメイン |
> | `xxx.web.app` | Firebase Hostingのドメイン |
>
> Cloud RunのURLは**自動では追加されない**ため、手動で追加が必要です。
