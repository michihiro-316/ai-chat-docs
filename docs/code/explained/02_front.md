...
!!! tip "レイアウトイメージ"
    ...
    `justify-content: space-between` で「AI チャット」が左、「👤 田中太郎 ログアウト」が右に分散配置される
...
!!! example "表示イメージ"
    ...
    | 位置 | クラス | 表示内容 |
    |------|--------|----------|
    | 左寄せ | `.assistant` | 「こんにちは！何でも聞いてください」 |
    | 右寄せ | `.user` | 「プログラミングを教えて」 |
    | 左寄せ | `.assistant` | 「プログラミングとは...」 |
...
!!! tip "仕組み"
    ...
    `border-top` だけ青色にして回転させる → くるくる回っているように見える（⟳ 考え中...）
...
!!! note "3つの画面がある"
    ...
    | 画面ID | 初期状態 | 表示タイミング |
    |--------|----------|----------------|
    | `login-screen` | 表示 | 最初に表示 |
    | `access-denied` | hidden | 権限がない時に表示 |
    | `chat-screen` | hidden | ログイン成功時に表示 |

    ※ 同時に表示されるのは1つだけ。script.js で hidden を付け外しして切り替える
...
!!! example "表示イメージ"
    ...
    **チャット領域：**
    ...
    - 「こんにちは！何でも聞いてください。」（assistant）
    - 「プログラミングを教えて」（user・右寄せ）
    - 「プログラミングとは...」（assistant）
