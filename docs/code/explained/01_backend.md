...
| 層 | 名前 | 何を守る？ | 突破されると？ |
|:--:|------|-----------|---------------|
| 1 | CORS | 不正サイトからのAPI利用 | 結果が盗まれる可能性 |
| 2 | JWT検証 | 未ログインユーザー | **課金される** |
| 3 | 許可リスト | 部外者のアクセス | **課金される** |

!!! warning "CORSだけでは不十分な理由"
...
!!! tip "この後のコードを読むポイント"
    ...
    | セクション | 対応する防御層 | 実行順 |
    |-----------|---------------|:------:|
    | 4-1. set_cors_headers | 第1層：CORS | 1 |
    | 5-2. get_email_from_token | 第2層：JWT検証 | 2 |
    | 5-1. is_email_allowed | 第3層：許可リスト | 3 |

    ※ セクション番号とコードの実行順は異なります。
    実際の処理は「JWT検証 → 許可リスト」の順番で行われます。

### なぜ「ヘッダー」で防御できるのか？

ヘッダーの話が出てくる前に、**HTTP通信の仕組み**を理解しましょう。
...
!!! warning "重要な注意"
    ...
    ```
    ブラウザ → ヘッダーに従う → 防御できる ✓
    curl/Postman → ヘッダー無視できる → 防御できない ✗
    ```

    だから**JWT検証や許可リスト**も必要なのです。
