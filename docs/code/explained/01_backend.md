...
CORSだけでは課金を防げないため、このシステムは**複数の防御層**で守られています。
...
| 層 | 名前 | 何を守る？ | 突破されると？ |
|:--:|------|-----------|---------------|
| 1 | CORS | 不正サイトからのデータ取得 | データが盗まれる可能性（課金は防げない） |
| 2 | JWT検証 | 未ログインユーザー | **勝手にAPIを使われ課金される** |
| 3 | 許可リスト | 部外者のアクセス | **勝手にAPIを使われ課金される** |

!!! example "具体例：各層が突破された場合"

    **第1層（CORS）だけ突破された場合：**
    ```
    evil.com → APIを叩く → 処理される → 課金発生！
                                    → でもデータはevil.comに渡らない
    ```
    → 課金されるが、データは守られる
