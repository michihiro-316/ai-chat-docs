...
!!! question "なぜ for じゃなくて try を使うの？"

    **for と try は目的が全く違います。**

    | 構文 | 目的 | 使う場面 |
    |------|------|---------|
    | `for` | **繰り返し** | リストの中身を1つずつ処理したい |
    | `try` | **エラー対策** | 失敗するかもしれない処理を安全に実行したい |

    **例：買い物リスト**

    ```python
    # for = リストの商品を1つずつレジに通す
    items = ['りんご', 'みかん', 'ぶどう']
    for item in items:
        scan(item)  # 1つずつスキャン

    # try = バーコードが読めなくても止まらないようにする
    try:
        scan(item)  # スキャンを試みる
    except:
        print('読み取り失敗')  # 失敗しても続行
    ```

    **get_email_from_tokenでは：**

    - リストを繰り返し処理する必要は**ない**（トークンは1つだけ）
    - でもトークンが壊れていたらエラーになる**かもしれない**

    だから `try` を使います。

    ```text
    for  → 「これを全部やって」（繰り返し）
    try  → 「これ試してみて、ダメだったらこっち」（失敗対策）
    ```
